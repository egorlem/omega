  ...
  u32 key_L = 0;
	...
	while(1) {
		...
		if (page_num==SD_list) {	//Load to PSRAM or NOR 
			...
			if (MENU_line<2) {
			//PSRAM DirectPSRAM or soft reset   or run NOR game 
		...
		if (page_num == SD_list) {	
			if (MENU_line < 2) { 
				//PSRAM DirectPSRAM or soft reset
